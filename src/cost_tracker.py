...
def compute_cost(model: str, in_tokens: int, out_tokens: int, cfg: Dict[str, Any]) -> float:
    """
    ...
    cost = (in_tokens / 1_000_000.0) * float(prices.get("input", 0)) + (out_tokens / 1_000_000.0) * float(
        prices.get("output", 0)
    )
    return round(cost, 8)
