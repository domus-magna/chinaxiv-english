...
class TranslationQAFilter:
    ...
    def __init__(self):
        ...
        self.MAX_CHINESE_IDEOGRAPH_RATIO = 0.001  # 0.1% max Chinese ideographs (any Chinese characters)
    ...
    def check_translation(self, translation: Dict[str, Any]) -> QAResult:
        ...
        for field in text_fields:
            ...
            field_ideograph_ratio = self.detector.calculate_chinese_ideograph_ratio(field_text)
            ...
            if field_chinese_chars:
                ...
                if field_ideograph_ratio > self.MAX_CHINESE_IDEOGRAPH_RATIO:
                    ...
                    issues.append(f"{field} contains Chinese characters: {field_chinese_chars[:5]}")
                
                # Flag if excessive Chinese punctuation
                elif field_chinese_ratio > self.MAX_CHINESE_PUNCTUATION_RATIO:
                    flagged_fields.append(field)
                    issues.append(f"{field} contains excessive Chinese punctuation: {field_chinese_chars[:5]}")
        ...
        all_text = ' '.join(str(translation.get(field, '')) for field in text_fields if translation.get(field))
        total_chinese_ratio = self.detector.calculate_chinese_ratio(all_text)
        total_ideograph_ratio = self.detector.calculate_chinese_ideograph_ratio(all_text)
        
        # Determine status
        if (total_ideograph_ratio > self.MAX_CHINESE_IDEOGRAPH_RATIO or 
            total_chinese_ratio > self.MAX_CHINESE_PUNCTUATION_RATIO or 
            self.detector.has_chinese_metadata(all_text)):
...
def filter_translations(translations: List[Dict[str, Any]], 
                      flag_for_review: bool = True) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
